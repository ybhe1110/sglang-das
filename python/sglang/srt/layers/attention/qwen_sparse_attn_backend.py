...
def _resolve_trtllm_sparse_decode():
    ...
    if not is_cuda() or not (is_sm100_supported() or is_sm121()):
        return None
    try:
...
class QwenSparseAttnBackend(AttentionBackend):
    ...
    def forward_extend(
        ...
        **kwargs,
        ...
        for i in range(len(sequence_lens)):
            slots = req_to_token[req_indices[i], : sequence_lens[i]].long()
            k_part, v_part = self._gather_qsa_token_kv(k_buffer, v_buffer, slots)
    ...
    def _qsa_kv_meta(self, k_buffer: torch.Tensor, v_buffer: torch.Tensor):
        """Describe the token-KV layout QSA must gather from.

        HCU FA remains the pool layout (pages, heads, page, dim) / (pages, heads, dim, page).
        QSA only maps that layout onto packed NHD rows; it does not change the pool.
        """
        page_size = int(getattr(self.token_to_kv_pool, "page_size", 1) or 1)
        if k_buffer.ndim == 3 and v_buffer.ndim == 3:
            return "nhd", int(k_buffer.shape[1]), int(k_buffer.shape[2]), page_size
        if k_buffer.ndim == 4 and v_buffer.ndim == 4:
            heads = int(k_buffer.shape[1])
            if (
                k_buffer.shape[2] == v_buffer.shape[3]
                and k_buffer.shape[3] == v_buffer.shape[2]
            ):
                return "hcu_fa", heads, int(k_buffer.shape[3]), int(k_buffer.shape[2])
            if tuple(k_buffer.shape[2:]) == tuple(v_buffer.shape[2:]):
                return "hnd", heads, int(k_buffer.shape[3]), int(k_buffer.shape[2])
        raise ValueError(
            "Unsupported QSA KV layout: "
            f"k={tuple(k_buffer.shape)} v={tuple(v_buffer.shape)}"
        )

    def _gather_qsa_token_kv(
        self, k_buffer: torch.Tensor, v_buffer: torch.Tensor, slots: torch.Tensor
    ):
        layout, _, _, page_size = self._qsa_kv_meta(k_buffer, v_buffer)
        slots = slots.long()
        if layout == "nhd":
            return k_buffer.index_select(0, slots), v_buffer.index_select(0, slots)
        page_idxs = slots // page_size
        offsets = slots % page_size
        if layout == "hcu_fa":
            return (
                k_buffer[page_idxs, :, offsets, :],
                v_buffer[page_idxs, :, :, offsets],
            )
        return (
            k_buffer[page_idxs, :, offsets, :],
            v_buffer[page_idxs, :, offsets, :],
    ...
    def _forward_trtllm_sparse(
        ...
        _, num_kv_heads, head_dim, _ = self._qsa_kv_meta(k_buffer, v_buffer)
        k_scale, v_scale = self._kv_descales(layer, k_buffer.dtype)
    ...
    def _forward_paged_attention(
        ...
        k_buffer = pool.get_key_buffer(layer.layer_id)
        v_buffer = pool.get_value_buffer(layer.layer_id)
        layout, num_kv_heads, head_dim, _ = self._qsa_kv_meta(k_buffer, v_buffer)
        if not q.is_cuda:
            if layout != "nhd":
                raise ValueError(
                    "QSA CPU reference path requires NHD token KV, "
                    f"got k={tuple(k_buffer.shape)} v={tuple(v_buffer.shape)}"
                )
            metadata = self._resolve_metadata(forward_batch)
            slots = self._logical_to_physical(topk_indices, metadata)
            scale_kwargs = {}
        ...
        # HIP packed-decode and the CUDA FA fallback both use tl.dot / FA on
        # query-dtype scratch. Widen FP8 cache on compact (with per-layer
        # descales) instead of feeding fp8e4nv into the MMA.
        k_scale, v_scale = self._kv_descales(layer, k_buffer.dtype)
        packed_dtype = (
            q.dtype
            if (is_hip() or is_fp8_kv_dtype(k_buffer.dtype))
            else k_buffer.dtype
        )
        packed_k, packed_v = self._get_fa2_scratch(
            scratch_capacity,
