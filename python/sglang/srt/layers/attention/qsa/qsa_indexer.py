...
class QSAIndexer(MultiPlatformOp):
    ...
    def update_key_state_and_compress(
        ...
        if is_extend:
            ...
            if is_hip():
                # Device-side write plan pads with no-op entries targeting
                # reserved compressed slot 0. A short HIP extend can have
                # fewer source rows than compress_ratio; clamp those gathers.
                n_src = max(int(token_k.shape[0]) - 1, 0)
                group_locs = torch.where(
                    (compressed_locs != 0)[:, None],
                    group_locs.clamp(max=n_src),
