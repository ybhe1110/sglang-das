...
def _launch_sparse_gqa_chunk_prefill(
    q,
    k,
    v,
    indices,
...
def sparse_gqa_packed_decode_triton(
    ...
    return _launch_sparse_gqa_chunk_prefill(
        q, k, v, indices, cu_q, cu_k, kv_lens, scale, 1, k_scale, v_scale
    )
...
def sparse_gqa_fwd_interface_triton_ck(
    ...
    return _launch_sparse_gqa_chunk_prefill(
        q, k, v, indices, cu_q, cu_k, kv_lens, scale, max_q, k_scale, v_scale
    )
...
def _compact_kv_hcu_fa(
    ...
    if DEQUANTIZE_FP8:
        k_values = k_values.to(tl.float32) * k_scale
        v_values = v_values.to(tl.float32) * v_scale
    tl.store(k_dst, k_values, mask=mask)
    tl.store(v_dst, v_values, mask=mask)
