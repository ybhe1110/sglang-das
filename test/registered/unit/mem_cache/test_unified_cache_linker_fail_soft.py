...
The fakes here are built against this branch's signatures: the linker ABC
requires ``pop_completed_offload`` (take/commit_completed_offloads live on
the wrapper, not the backend), ``_update_load`` reads ``component_type``
on the COMMIT path, and ``load_back`` calls
``tree_core.collect_full_device_indices`` and then walks ``node.parent``
up to the request's pre-load node.
"""
...
class _FakeLinkerBase(UnifiedCacheLinker):
    """Minimal backend implementing exactly the linker ABC."""
...
class _RevalidatingFakeLinker(_FakeLinkerBase):
    """Backend whose revalidate_load verdict is scripted per test."""
...
class _LegacyFakeLinker(_FakeLinkerBase):
    """Backend that has not implemented the revalidation hook at all."""
...
def _cache_for_load_back(component, empty_indices):
    lock_params = object()
    # insert publishes node 9; its parent is the request's pre-load node (5),
    # which is where load_back stops marking external_cache_stored.
    node5 = SimpleNamespace(id=5, parent=None, external_cache_stored=False)
    node9 = SimpleNamespace(id=9, parent=node5, external_cache_stored=False)
    nodes = {5: node5, 9: node9}
    cache = SimpleNamespace(
        tree_core=SimpleNamespace(
            enable_external_cache_linker=False,
            empty_match_result=SimpleNamespace(device_indices=empty_indices),
            # The two tail pages the load publishes, in root order.
            collect_full_device_indices=lambda from_id, until_id: torch.arange(
                2, dtype=torch.int64
            ),
        ),
        ...
        # prefix_len is 4 over a 2-page tail, so the adopted range is [2, 4).
        insert=lambda params: SimpleNamespace(
            last_device_node=9,
            mamba_exist=False,
            adopted_ranges={ComponentType.FULL: [(2, 4)]},
        ),
        inc_lock_ref=lambda node_id: SimpleNamespace(
            to_dec_params=lambda: lock_params
        ),
        dec_lock_ref=lambda node_id, params: None,
        resolve_node_handle=lambda node_id: nodes[node_id],
...
def test_backend_without_revalidation_keeps_legacy_path():
    empty_indices = torch.zeros(0, dtype=torch.int64)
    component = _LoadBackComponent()
    # A backend that has not implemented the hook.
    linker = _LegacyFakeLinker()
    assert not hasattr(linker, "revalidate_load")
    cache, lock_params = _cache_for_load_back(component, empty_indices)
    wrapper = UnifiedCacheLinkerWrapper(cache, linker)
    wrapper.hit_markers["rid"] = _hit_marker()
